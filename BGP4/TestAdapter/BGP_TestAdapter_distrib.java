 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class BGP_TestAdapter_distrib extends TestAdapter implements TriCommunicationSA,TriPlatformPA, TciEncoding 
{
	...
	public BGP_TestAdapter_distrib()
	{
		super();
	}
	...
	public TriStatus triMap(final TriPortId compPortId,final TriPortId tsiPortId) //throws IOException
	{
		...
		*/
		if (tsiPortId.getPortName().equals("BGP_Interface_Port"))
		{
			...
			{
				//2005.09.23
				String router_interface_ipaddress=(String)RB.PortAddress.get("BGP_Interface_Port");
				//InetAddress	router_addr = InetAddress.getByName("192.168.3.1"); //2005.09.23
				InetAddress	router_addr = InetAddress.getByName(router_interface_ipaddress); //2005.09.23
				int	router_port = 179;
				
				rxSocket_pco1 = new Socket(router_addr,router_port);
				RB.logging.logDatapackets("\n rxSocket_pco1 = new Socket(router_addr,router_port) OK!!!");
				runThread_pco1 = true;
				...
				RB.logging.logDatapackets("\n  tcpReceiverThread_pco1 = new Thread() OK!!!");
				tcpReceiverThread_pco1.start(); 
				RB.logging.logDatapackets("\n  tcpReceiverThread_pco1.start() OK!!!");
				return new TriStatusImpl();
			}
		...
		if(tsiPortId.getPortName().equals("BGP_Interface_Port2"))
		{
			...
			try
			{
				//2005.09.23
				/*
				InetAddress	router2_addr = InetAddress.getByName("192.168.2.1");
				*/
				String router_interface_ipaddress2=(String)RB.PortAddress.get("BGP_Interface_Port2");//2005.09.23
				InetAddress	router2_addr = InetAddress.getByName(router_interface_ipaddress2); //2005.09.23
				
				int	router2_port = 179;
				
				rxSocket_pco2 = new Socket(router2_addr,router2_port);
				RB.logging.logDatapackets2("\n rxSocket_pco2 = new Socket(router2_addr,router2_port) OK!!!");
				runThread_pco2 = true;
				...
				RB.logging.logDatapackets2("\n tcpReceiverThread_pco2 = new Thread() OK!!!");
				tcpReceiverThread_pco2.start(); 
				RB.logging.logDatapackets2("\n tcpReceiverThread_pco2.start() OK!!!");
				return new TriStatusImpl();
			}
			catch (SocketException sex)
			{
		...
		if(tsiPortId.getPortName().equals("BGP_Interface_Port3"))
		{
			...
			try
			{
				...
				*/
				String router_interface_ipaddress3=(String)RB.PortAddress.get("BGP_Interface_Port3");//2005.09.23
				InetAddress	router3_addr = InetAddress.getByName(router_interface_ipaddress3); //2005.09.23
	...
	*/
	  if (tsiPortId.getPortName().equals("BGP_Interface_Port"))
	    ...
	    private byte[]  HandleNotification(byte[] buf)
	    {
		...
		{
				return "BGP_TestAdapter_distrib";
		}
	    
}// end public class BGP_TestAdapter_distrib
