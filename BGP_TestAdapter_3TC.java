	...
	*/
	public TriStatus triMap(final TriPortId compPortId,final TriPortId tsiPortId) //throws IOException
	{
		...
		if (tsiPortId.getPortName().equals("BGP_Interface_PortA"))
		{
			...
			try
			{
				InetAddress	router_addr = InetAddress.getByName("2001:0:1::6");
				int	router_port = 179;
				...
				tcpReceiverThread_pco1 = new Thread()
				{
					public void run() 
					{
							...
							while (mylock) 
							{
								...
								try 
								{
									...
									if (length > 0) 
									{
										...
										RB.logging.logDatapackets("\n In the BGP_Interface_PortA, the Received Thread:received message length:"+ rcvMessage.getEncodedMessage().length);
										synchronized (threadlock) 
										{
											if (runThread_pco1) 
											{
						...
						{
							RB.logging.logDatapackets("\n"+"MyTestAdapter: the BGP_Interface_PortA: SocketException "+se.toString());
						}
					}
				};
		...
		if(tsiPortId.getPortName().equals("BGP_Interface_PortB"))
		{
			RB.logging.logDatapackets2("\n Current port:"+tsiPortId.getPortName()); 
			// prepare to be ready to communicate           
			try
			{
				InetAddress	router2_addr = InetAddress.getByName("2001:0:2::6");
		...
		if(tsiPortId.getPortName().equals("BGP_Interface_PortC"))
		{
			RB.logging.logDatapackets("\n Current port:"+tsiPortId.getPortName()); 
			// prepare to be ready to communicate           
			try
			{
				InetAddress	router3_addr = InetAddress.getByName("2001:0:3::6");
